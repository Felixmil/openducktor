...
The host selects the shell, arguments, and clean child environment. The renderer cannot choose an executable, arguments, or environment variables. On Unix, resolve the login shell from the user account and fall back to `SHELL`, then use it with `TERM=xterm-256color` and `COLORTERM=truecolor`.
